...
export default class extends React.Component {
  ...
  getWeather = async(potato, longitude) =>{  
    const { data } = await axios.get(`http://api.openweathermap.org/data/2.5/weather?lat=${potato}&lon=${longitude}&appid=${API_KEY}&units=metric`)  //openWeather Doc에 unit of measurement를 보면 &units=metric를 url에 추가해야 섭씨로 나온다
